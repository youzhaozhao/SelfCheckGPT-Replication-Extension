...
class SelfCheckNLI:
    """
    SelfCheckGPT (NLI variant): 完全对齐原论文逻辑，修复logits筛选+异常处理问题
    """
    def __init__(
        self,
        nli_model: str = None,
    ...
    def predict(
        ...
        """
        完全对齐原论文逻辑：
        1. 不手动筛选logits（模型已移除neutral类，logits维度=2）
        2. 仅捕获严重错误（避免大量0.5兜底拉低指标）
        3. 直接使用模型输出概率（无需强制范围限制）
        """
        num_sentences = len(sentences)
        num_samples = len(sampled_passages)
        scores = np.zeros((num_sentences, num_samples))
        # 统计失败的(sentence, sample)配对数（用于后续验证）
        fail_count = 0
            ...
            for sample_i, sample in enumerate(sampled_passages):
                try:  
                    inputs = self.tokenizer.batch_encode_plus(
                        batch_text_or_text_pairs=[(sentence, sample)],  # 原论文输入顺序一致，无需修改
                        add_special_tokens=True, padding="longest",
                        truncation=True, return_tensors="pt",
                        return_token_type_ids=True, return_attention_mask=True,
                    )
                    inputs = inputs.to(self.device)
                    logits = self.model(** inputs).logits  # 关键修改：不筛选维度，直接使用原始logits
                    
                    # 验证logits维度（原论文应为2维，对应entailment/contradiction）
                    if logits.shape[-1] != 2:
                        print(f"警告: 模型logits维度={logits.shape[-1]}，预期=2（请确认NLI模型是移除neutral的版本）")
                        fail_count += 1
                        continue

                    probs = torch.softmax(logits, dim=-1)
                    prob_contradict = probs[0][1].item()  # 原论文固定取索引1为contradiction概率
                    scores[sent_i, sample_i] = prob_contradict

                except Exception as e: 
                    # 仅在严重错误时跳过该配对（而非返回0.5），避免拉低整体指标
                    print(f"样本({sent_i},{sample_i})处理失败: {str(e)[:100]}")
                    fail_count += 1
                    continue

        # 打印失败率（正常应<5%，若过高需检查模型或数据）
        total_pairs = num_sentences * num_samples
        print(f"SelfCheckNLI: 总配对数={total_pairs}，失败数={fail_count}，失败率={fail_count/total_pairs:.2%}")

        # 计算每个句子的平均矛盾概率（与原论文一致）
        scores_per_sentence = scores.mean(axis=-1)
        return scores_per_sentence

class SelfCheckPrompt:
    """
    SelfCheckGPT (Prompt variant): Checking LLM's text against its own sampled texts via LLM prompting
    按照原论文方法：将句子与每个采样段落比较，问是否支持
    """
    def __init__(
        self,
...
Answer Yes or No:"""
        ...
        print(f"SelfCheck-Prompt ({model_name}) initialized to device {self.device}")
    ...
    def predict(
        ...
        """
        原论文的Prompt方法：将每个句子与每个采样段落比较
        :param sentences: 要评估的句子列表
        :param sampled_passages: 采样段落列表（证据）
        :return: 句子级别的幻觉分数（越高越可能是幻觉）
        """
        num_sentences = len(sentences)
        num_samples = len(sampled_passages)
        ...
        for sent_idx, sentence in enumerate(tqdm(sentences, disable=disable, desc="SelfCheck-Prompt")):
            sample_scores = []
            ...
            for sample_idx, sample_passage in enumerate(sampled_passages):
                try:
                    ...
                    # 只取生成的部分（去掉prompt）
                    generated_text = answer_text[len(prompt):].strip()
                    
                    score = self._parse_answer(generated_text)
                    sample_scores.append(score)
                    
                except Exception as e:
                    print(f"Prompt处理失败 (句子{sent_idx}, 样本{sample_idx}): {e}")
                    sample_scores.append(0.5)  # 出错时使用中性分数
            
            # 对每个句子的所有采样段落分数取平均（原论文方法）
            if sample_scores:
                sent_score = np.mean(sample_scores)
                sent_scores.append(sent_score)
            else:
